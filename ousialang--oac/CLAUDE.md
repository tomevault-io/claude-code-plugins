...
```
Read and follow the instructions in .claude/skills/oac/SKILL.md
```
...
- Namespaces are top-level and declaration-only: `namespace TypeName { fun helper(...) -> ... { ... } }`, `namespace TypeName { comptime fun helper(...) -> ... { ... } }`, and `namespace TypeName { extern fun symbol(...) -> ... }`. Namespace calls use `TypeName.helper(...)` syntax and lower to internal lookup names using `TypeName__helper`.
...
- Stdlib sources and execution fixtures now prefer receiver method syntax whenever the helper's first parameter is the owning concrete type or specialization alias (`text.equals(other)`, `table.get(key)`, `vec.push(x)`); this now also applies inside comptime-evaluated stdlib helpers where the comptime surface supports the same receiver-first helper shape (for example `enum_opt.is_some()` in `std_comptime.oa`). Keep static namespace form for constructors, helpers whose receiver is not parameter 0 (for example `push_front(value, list)`), trait dispatch, and explicit namespace-vs-method syntax coverage.
...
- Non-extern functions may declare an optional multiline precondition block between the return type and body: `fun f(x: I32) -> I32 pre { x > 5 } { ... }` and `comptime fun f(x: I32) -> I32 pre { x > 5 } { ... }`. `pre` blocks require at least one Bool clause, remain verification-only for runtime functions, and are rejected on `extern fun` and trait method declarations; comptime function preconditions are checked at comptime call sites.
- Front-end resolution now runs in two internal modes: a bootstrap comptime mode that loads stdlib, expands generics, collects type/function metadata, normalizes receiver-method and operator-trait calls, type-checks runtime+comptime bodies, and indexes semantic-expression metadata without requiring `main`; then a final runtime mode that reuses the same staged builder before ownership rewriting, runtime-only semantic pruning/rejection, model-invariant purity validation, and `main` validation.
...
- Enum payload variants use colon syntax in definitions (`Ok: I32`, `Err: ParseErr`); unit variants stay bare (`Done`).
- Comptime-emitted or inline test/source fixtures use the same enum payload syntax rules as ordinary source: payload variants must be written with `:` and legacy `Variant(Type)` forms are parse errors everywhere.
...
- Struct equality is universal bytewise comparison only as the fallback when no explicit `Equality` impl exists: `==` / `!=` lower to `memcmp` over full struct size (including pointer-containing structs).
- Tag-only enum equality also has a builtin fallback, while tagged payload enums require an explicit `Equality` impl for `==` / `!=`.
- The stdlib entrypoint `crates/oac/src/std/std.oa` is now an import aggregator over split files in `crates/oac/src/std/` (`std_ascii.oa`, `std_char.oa`, `std_null.oa`, `std_meta.oa`, `std_comptime.oa`, `std_option_result.oa`, `std_string.oa`, `std_ref.oa`, `std_collections.oa`, `std_set.oa`, `std_vec.oa`, `std_traits.oa`, `std_json.oa`, `std_clib.oa`, `std_io.oa`).
- `crates/oac/src/std/std_traits.oa` defines core traits (`Hash`, `Addition`, `Subtraction`, `Multiplication`, `Division`, `Equality`, `Comparison`, `Copy`, `Drop`), concrete scalar `Copy` impls (`Bool`, `U8`, `I32`, `I64`, `FP32`, `FP64`, `Char`, `AsciiChar`), and std-owned operator impls for `Char`, `AsciiChar`, and `String` where needed.
...
- Legacy runtime helper builtins `sum`, `sub`, `eq`, `lt`, `string_len`, `slice`, and `print_str` are hard-removed from compiler seeding; arithmetic uses operators, while `string_len`, `slice`, and `print_str` now live as ordinary stdlib functions in `crates/oac/src/std/std_string.oa`.
...
- `crates/oac/src/std/std_string.oa` also now defines top-level string helpers `string_len(s) -> I32`, `slice(s, start, len) -> String`, and `print_str(s) -> I32`; `slice` carries explicit preconditions for non-negative in-bounds ranges and `len < 2147483647`, while `print_str` writes exactly the string bytes with no implicit newline.
...
- The stdlib `HashTable[K: Hash + Equality, V]` in `crates/oac/src/std/std_collections.oa` is now a dynamically resizing separate-chaining map (persistent value semantics) with APIs `new`, `with_capacity`, `set` (`SetResult { table, inserted_new }`), `get`, `remove` (`RemoveResult { table, removed }`), `len`, `capacity`, `contains_key`, and `clear`; fixed-size `put`/`size` APIs were removed.
- `HashTable[K: Hash + Equality, V]` currently has no declaration-based struct invariant; the previous `coherent_state` invariant was removed after fail-closed `solver unknown` obligations on the resize/rehash path, and behavior is now covered by runtime execution snapshots.
...
- The stdlib now also defines generic `HashSet[K: Hash + Equality]` in `crates/oac/src/std/std_set.oa` as a persistent separate-chaining set with APIs `new`, `with_capacity`, `insert` (`InsertResult { set, inserted_new }`), `remove` (`RemoveResult { set, removed }`), `contains`, `len`, `capacity`, `clear`, `union`, `intersection`, and `difference`.
...
- `crates/oac/src/std/std_meta.oa` now exposes the public comptime semantic API as ordinary namespace/method helpers instead of user-facing global builtins: `Type.name/resolve/is_struct/is_enum/as_struct_opt/as_enum_opt`, `StructInfo.field_count/field_at`, `EnumInfo.variant_count/variant_at`, `FieldInfo.name/ty`, `VariantInfo.name/payload_type_opt`, `DeclSet.new/add_empty_enum/add_enum_tag_variant/add_enum_payload_variant/add_derived_struct/add_invariant_field_gt_i32`, `SemanticExpr.definition_location_opt`, and `SourceSpan.display_compact`. Canonical semantic option aliases live there too (`TypeOpt`, `StructInfoOpt`, `EnumInfoOpt`, `FieldInfoOpt`, `VariantInfoOpt`, `SemanticExprOpt`, `SourceSpanOpt`).
- Ordinary stdlib helpers now provide the shared comptime/runtime surface where possible: comptime code prefers `opt.is_some()`, `opt.unwrap()`, and `name.concat("Suffix")`, backed by ordinary `Option`/`String` helpers in `std_option_result.oa` and `std_string.oa`. `Meta.expr_opt(x)` remains the only compiler-backed public exception because the source type system still cannot express its “any expression” parameter shape.
- Comptime semantic reflection/emission remains comptime-only under the hood and is rejected from runtime functions, but user source now reaches it through namespaced wrappers instead of raw builtin spellings. `comptime apply` now executes against the bootstrap `ResolvedProgram`, not an AST-only eval world, so applies reuse resolve-time stdlib loading, generic expansion, signature/type resolution, receiver-method normalization, operator-trait lowering, and semantic-expression indexing; after each emitted `DeclSet`, the compiler merges those AST declarations and bootstrap-resolves again so later applies can see earlier emitted declarations.
- Because comptime now runs on normalized resolved calls, applies can call imported helpers, stdlib wrappers such as `derive_enum_tags(T)` in `crates/oac/src/std/std_comptime.oa`, ordinary pure helpers with `pre` blocks, receiver methods, trait-operator lowered helper paths, `match`, struct literals/field access, enum payload matching, recursion (fuel-limited), scalar operations beyond `I32`, and debug-output sinks `print(...)` / `print_str(...)` (their output is discarded and they deterministically return `0`); other runtime-only operations that the evaluator cannot model still fail closed (for example externs and pointer load/store builtins).
...
- `oac build` / `oac test` stage failures are now mapped to stable diagnostic codes (for example `OAC-PARSE-001`, `OAC-RESOLVE-001`, `OAC-PRE-001`, `OAC-INV-001`, `OAC-MINV-001`, `OAC-LINK-001`, `OAC-LINK-002`) and emitted as Ariadne reports; execution fixture compilation-error snapshots therefore reflect Ariadne plain-report text.
...
- Repo-local proof summaries are cached under `target/oac/verification_cache/` as content-hash-keyed JSON summaries; ordinary-function summaries combine all local function-precondition, `prove(...)`, integer-safety, and struct-invariant obligations for one root function, while model invariants cache one summary per invariant checker function.
- Ordinary-function proof summaries now also include function-precondition and integer-safety obligations, so one cache hit for a root function simultaneously trusts call-site precondition checks, explicit `prove(...)`, reachable source-level integer arithmetic checks, and struct-invariant return-site checks for that root.
...
- The benchmark full suite now also includes model-invariant fixtures (`model_invariant_pass`, `model_invariant_fail`) plus a representative function-precondition failure fixture (`precondition_fail`); benchmark proof-stage artifact metrics include both `prove/*.smt2` and `preconditions/*.smt2`, while invariant metrics aggregate `struct_invariants/*.smt2` and `model_invariants/*.smt2`.
...
- Function `pre { ... }` clauses are anonymous Bool expressions that are conjuncted, synthesized into hidden helpers named `__pre__<function_name>__clause_<ordinal>`, and checked at every reachable user-defined call site from `main`, generated precondition helpers, struct-invariant helpers, and model-invariant roots; violations fail compilation with `OAC-PRE-001`.
...
- Model invariants are resolve-validated as a strict pure subset: transitive user-call graph traversal rejects `prove`/`assert`, extern calls, pointer load/store builtins, and side-effect builtins (`print`); std `print_str` is rejected transitively because it calls extern `Clib.write`.
- During `oac build`, verification obligations are orchestrated by `crates/oac/src/verification.rs`: under `check proofs` it runs function-precondition verification first, then `prove(...)`, then integer-safety verification; after that it runs struct-invariant verification and finally model-invariant verification.
- During `oac build`, function preconditions are verified per `(call-site, precondition clause)` pair by synthesizing per-site QBE checker functions from compiled QBE: the target call site is replaced with a call to the synthesized helper immediately before the original callee would execute, and checker exit is `1` on violated precondition / `0` on success (`unsat` passes, `sat` fails). Checker artifacts are emitted under `target/oac/preconditions/`.
...
- During `oac build`, source-level integer arithmetic obligations are also verified in `check proofs`: `qbe_backend` emits lightweight `.oac_integer_site_<type>__<op>__<path>` marker copies around integer `+/-/*//` expressions using shared AST-path-derived ids, `integer_safety.rs` collects sites reachable from `main` and all struct/model-invariant roots, and per-site checker artifacts are emitted under `target/oac/integer_safety/`. Integer-safety checkers use the same shared target-reachability pruning on the cloned caller CFG before checker-module closure.
...
- Prepared verification sites now keep two SMT variants: `solver_smt` (comment-free script used for solver execution, proof-cache hashing, summary inputs, and outcome tracking) and `artifact_smt` (human-readable `.smt2` written to disk with comments, readable relation/variable names, and source-linked headers).
...
- Checker encoding now also adds function-entry preconditions for callee-local `pre { ... }` clauses: for each checker function with synthesized precondition helpers, `qbe-smt` assumes each corresponding helper summary relation holds and returns non-zero at entry (assumption-only; entry memory/heap state is not replaced with helper-call outputs).
...
- Recursion-cycle rejection now operates on all checker entry-assumption edges, not just struct-argument invariant edges: call-only cycles are still allowed, but SCCs containing either arg-invariant assumptions or function-precondition assumptions fail closed.
...
- `qbe-smt` now exposes both solver and artifact scripts (`qbe_module_to_annotated_smt_with_assumptions`): solver scripts stay comment-free and stable for hashing, while artifact scripts add full-line `;` comments, readable relation names (`pc__...`, `ret__...`, `abort__...`), readable state variable names (`regs_curr`, `regs_in`, `arg_inv_ret_0`, `precond_ret_0`, ...), function-section comments, and per-rule PC/block/QBE-statement comments.
...
- Old key ops in generic hash tables: direct integer hash/`==` -> New: trait-bounded hashing plus `Equality`-backed infix equality (`Hash.hash(k)`, `a == b`) with bounds like `[K: Hash + Equality, V]`
