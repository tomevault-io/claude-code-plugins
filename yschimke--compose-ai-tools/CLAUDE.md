...
```
Read and follow the instructions in .claude/skills/compose-ai-tools/SKILL.md
```
...
- **Git history must never attribute work to an AI agent — non-negotiable.** Commits are attributed solely to the human committer. Banned: (1) a `Co-authored-by:` trailer naming an agent (Claude / Codex / ChatGPT / Copilot / Gemini) or using an agent no-reply email, and (2) committing with an **author/committer email that is an agent** (`noreply@anthropic.com` / `noreply@openai.com`). Fine and left untouched: links to an agent session (`claude.ai/code`), "🤖 Generated with …" footers, and ordinary PR comments — they don't claim authorship. PRs are squash-merged from the **PR title + body**, so a `Co-authored-by:` line in the PR description reaches `main` even when the commits are clean; scrub the description too. Enforced by the local [`.githooks/commit-msg`](../.githooks/commit-msg) hook and the [`No Agent Attribution`](../.github/workflows/no-agent-attribution.yml) CI gate — keep the shared `AGENT_NAME` / `AGENT_EMAIL` definitions in sync between them. Real human co-authors and `dependabot`/`renovate`/`github-actions` bot trailers are fine — the gate targets AI coding agents only.
